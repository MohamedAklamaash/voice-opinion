...
const MainHome = ({ primaryTheme }: Props) => {
  ...
  const [socialRooms, setSocialRooms] = useState<Room[]>([]);
  ...
  const getSocialRooms = async () => {
    if (!email) return;
    try {
      const { data: { data } } = await axios.get(`${import.meta.env.VITE_API_URL}/room/social/${encodeURIComponent(email)}`);
      setSocialRooms(data ?? []);
    } catch {}
  };

  useEffect(() => { getAllPublicRooms(); activateUser(); getPendingInvites(); getSocialRooms(); }, []);
    ...
    <div style={{ background: "var(--ink)" }}>
      ...
      <div className="max-w-5xl mx-auto px-5 py-8">
        ...
        <div className="flex items-end justify-between mb-8 flex-wrap gap-4">
          <div>
            <p className="font-mono text-xs tracking-widest mb-2" style={{ color: "var(--ash)" }}>— LIVE ROOMS —</p>
            <h1 className="font-bebas leading-none" style={{ fontSize: "clamp(2.5rem, 6vw, 4rem)", color: "var(--paper)" }}>
              HEY, <span style={{ color: "var(--gold)" }}>{userName.toUpperCase()}</span>
            </h1>
            <p className="font-mono text-xs mt-1" style={{ color: "var(--ash)" }}>What do you want to talk about today?</p>
          </div>
          <button
        ...
        {socialRooms.length > 0 && (
          <div className="mb-8">
            <p className="font-mono text-xs tracking-widest mb-4" style={{ color: "var(--ash)" }}>
              — FRIENDS' ROOMS · {socialRooms.length} —
            </p>
